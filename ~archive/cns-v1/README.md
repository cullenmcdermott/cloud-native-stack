...
Interested in deploying NVIDIA Cloud Native Stack? This repository has [install guides](install-guides) for manual installations and [Ansible playbooks](playbooks) for automated installations.
...
- CNS comes as a reference architecture that lists all components that have been tested successfully together. The CNS reference architecture can be used as specification for production deployments.
- CNS also comes as installation guides and playbooks that can be used to instantiate a quick K8s environment with NVIDIA operators. The CNS installation guides and playbook are intended only for test and PoC environments.

  Note: The K8s layer that CNS install guide or playbook deploys is basic (no HA for instance) and as such cannot be used for production. However all NVIDIA components in CNS are fully operable in production environments.
...
`NOTE:` CNS Version 15.0 and above now supports Ubuntu 24.04

For more information, refer to the [Cloud Native Stack Releases](https://github.com/NVIDIA/cloud-native-stack/releases).
...
> Note: Previous Cloud Native Stack release information can be found [here](https://github.com/NVIDIA/cloud-native-stack/tree/24.11.1?tab=readme-ov-file#nvidia-cloud-native-stack-component-matrix).

`NOTE:` Cloud Native Stack versions are available with the main branch but it's recommended to use the specific branch.
...
- [Installation on CSPs](playbooks/readme.md#installation-on-csps)
...
- [KServe](playbooks/readme.md#enable-kserve-on-cns)
- [LeaderWorkerSet (lws)](playbooks/readme.md#enable-leaderworkerset)
...
| Load Balancer | MetalLB: 0.15.2 | MetalLB: 0.15.2 | MetalLB: 0.15.2 |
...
- System should have an operating system either Ubuntu 22.04/24.04 or newer.
...
- System has only one network interface configured with internet access. The IP is static and doesn't change.
- UEFI secure boot is disabled.
- Root file system has at least 40GB capacity.
- System has 2 CPU cores and 4GB memory.
...
Update the hosts file in docs/playbooks directory with master and worker nodes (if you have) IPs with username and password like below:
...
Install the NVIDIA Cloud Native Stack by running the below command. "Skipping" in the Ansible output refers to the Kubernetes cluster being up and running.

```
bash setup.sh install
```

For more information about customizing the values, please refer to the [Installation](playbooks/readme.md#installation) guide.

# Topologies

- Cloud Native Stack allows deployment of:
